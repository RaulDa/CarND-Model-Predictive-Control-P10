...
// vector start position for state variables and actuator inputs
size_t x_start = 0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // she upper and lower limits of delta are set to -25 and 25
  ...
  // extract state, actuator inputs and predicted trajectory
  vector<double> output;
  output.push_back(solution.x[psi_start + 1]);
  output.push_back(solution.x[v_start + 1]);
  output.push_back(solution.x[cte_start + 1]);
  output.push_back(solution.x[epsi_start + 1]);
